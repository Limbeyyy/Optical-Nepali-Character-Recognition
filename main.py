...
class LabelSmoothingLoss(nn.Module):
    def __init__(self, classes=58, smoothing=0.05):
...
def train_models(config, train_dataset, val_dataset, device, logger):
    ...
    for model_idx in range(num_models):
        ...
        model = EnhancedBMCNNwHFCs(num_classes=58, dropout_rate=config['dropout']).to(device)
        ...
        criterion = LabelSmoothingLoss(classes=58, smoothing=config.get('label_smoothing', 0.05))
...
def evaluate_model(model, test_loader, device, criterion, logger, num_classes=58):
    model.eval()
    test_loss = 0.0
    correct = 0
    total = 0
...
def main():
    ...
    train_dataset = NepaliMNISTDataset(
        data_dir='/home/kataho/MallaNet/data/extracted/Train',
        transform=train_transform,
        logger=logger
    )
    test_dataset = NepaliMNISTDataset(
        data_dir='/home/kataho/MallaNet/data/extracted/Test',
    ...
    model = EnhancedBMCNNwHFCs(num_classes=58, dropout_rate=best_config['dropout']).to(device)
    ...
    criterion = LabelSmoothingLoss(classes=58, smoothing=best_config['label_smoothing'])
